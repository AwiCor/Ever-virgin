...
(function () {
  ...
  function buildSteps(prayers, mysterySet, sessionPicks, saintPrayer, saintBio) {
    const steps = [];
   ...
   *  Clears the fallback overlay and rejects extremely tall/narrow paintings
   *  that would otherwise render as a thin vertical strip. */
  function _onPaintingLoad(imgEl, frameId) {
    const frame = imgEl.closest(".painting-frame");
    if (!frame) return;
    // The image loaded successfully — hide the title/artist fallback text.
    frame.classList.remove("has-fallback");

    // Reject extremely portrait paintings (width:height < 0.55, i.e. taller
    // than ≈ 1.8× wide) — they look like a thin strip in the gold frame.
    // Try the next alternative; the new img's onload will check again.
  ...
  function fitAnnouncementPainting() {
    ...
    // Reserve a generous safety buffer so the painting never sits flush
    // against the surrounding text or the stage's padding, and to absorb
    // any height-grow that happens after this pass (e.g. scripture
    // reflowing when the painting loads at its final width). The CSS flex
    // gap supplies the visible spacing between siblings; this just adds
    // headroom on top.
    const target = Math.max(180,
      availableVertical - usedByText - frameOverhead - 60);

    img.style.maxHeight = (target * imageScale) + "px";
  }
  ...
  async function startPrayerPage() {
    ...
    const steps = buildSteps(data.prayers, mysterySet, sessionPicks, saintPrayer, saintBio);
    ...
    function show(idx) {
      ...
      // On the mystery-announcement screen, run a JS sizing pass that
      // gives the painting whatever vertical room is left after the actual
      // rendered text. Two-phase: first on layout, then again after the
      // image's intrinsic dimensions are known (the painting's aspect can
      // affect text wrapping margins).
      if (step.type === "announce") {
        requestAnimationFrame(fitAnnouncementPainting);
      ...
      // Auto-hide the keyboard legend once, after the user has navigated past
      // the introductory Our Father. After that, the user controls visibility
      // via the chevron toggle in the bottom-right.
      if (!kbHintAutoHidden && introOurFatherIdx >= 0 && cursor > introOurFatherIdx) {
        kbHintAutoHidden = true;
        setKbHintHidden(true);
      }

      // Restart the auto-advance timer for the new step.
      scheduleAutoAdvance();
    ...
    document.addEventListener("click", (e) => {
      ...
      if (e.target.closest(".kb-hint-toggle")) return; // legend toggle button
      next();
    });
