...
(function () {
   ...
   *              "glory_fatima", "announce", "hail_holy_queen" (which now
   *              also carries the closing versicle & response),
   *              "closing_prayer", "st_michael", "saint_of_day",
   *              "closing_sign", "amen"
   */
  ...
  // `movements` is an array of { mysteryKey, mysterySet, sessionPicks }.
  // One movement = the classic five-decade Rosary. Three movements
  // (Joyful, Sorrowful, Glorious) = the traditional fifteen-decade Rosary:
  // the opening and closing prayers are said once, the five-bead loop is
  // circled once per set.
  function buildSteps(prayers, movements, saintPrayer, saintBio) {
    const steps = [];

    // The three call-and-response prayers, normalised once up front.
    const ofParts = splitPrayer(prayers.our_father);
    const hmParts = splitPrayer(prayers.hail_mary);
    const glParts = splitPrayer(prayers.glory_be);
    const ffParts = splitPrayer(prayers.fatima_prayer);
    ...
    // For each movement, for each of its 5 decades:
    ...
    movements.forEach((mv, mi) => {
      const isLastMovement = mi === movements.length - 1;
      for (let d = 1; d <= 5; d++) {
        const mystery = mv.mysterySet.mysteries[d - 1];
        const pick = mv.sessionPicks[d - 1];
        const ofBead = "bead-d" + d + "-of";

        const decadeCtx = {
          number: d,
          mysteryName: mystery.name,
          mysterySet: mv.mysteryKey, // "joyful" | "luminous" | "sorrowful" | "glorious"
          fruit: mystery.fruit,
        ...
        // The Glory Be + Fatima prayer lights the NEXT decade's OF bead, as
        // on a physical rosary. Within a set that's the next bead; at the
        // end of a set that isn't the last, the loop wraps back to the
        // first OF bead (the rosary is circled again); at the very end of
        // the whole Rosary it rests on the final Hail Mary bead.
        let gloryFatimaBead;
        if (d < 5) gloryFatimaBead = "bead-d" + (d + 1) + "-of";
        else if (!isLastMovement) gloryFatimaBead = "bead-d1-of";
        else gloryFatimaBead = "bead-d5-hm-10";

        steps.push({
          type: "glory_fatima",
          gloryLead: glParts.lead,
          gloryResp: glParts.resp,
          fatimaLead: ffParts.lead,
          fatimaResp: ffParts.resp,
    ...
    // Closing: Hail Holy Queen, followed on the SAME screen by its
    // versicle & response — they are one liturgical unit, so they're
    // never split across two pages. Medallion bead.
    steps.push({
      type: "hail_holy_queen",
      text: prayers.hail_holy_queen,
      versicleLeader: prayers.closing_versicle.leader,
      versicleResponse: prayers.closing_versicle.response,
  ...
  /**
   * A prayer in prayers.json is either a plain string (said by everyone
   * together) or a { lead, resp } object — the traditional led-Rosary
   * call-and-response, where the leader prays `lead` and the group answers
   * with `resp`. Normalises both shapes to { lead, resp }.
   */
  function splitPrayer(p) {
    if (p && typeof p === "object") {
      return { lead: p.lead || "", resp: p.resp || "" };
    }
    return { lead: typeof p === "string" ? p : "", resp: "" };
  }

  /**
   * Renders a prayer's text. If it has a leader/response split, it reads as
   * one continuous prayer — the only distinction is colour: the leader's
   * words in the normal cream, the group's response in warm gold, so on a
   * shared screen everyone sees when to join in without any labels.
   */
  function leadRespHTML(lead, resp) {
    if (resp && resp.trim()) {
      return `<p class="prayer-text">${escapeHTML(lead)} <span class="prayer-resp">${escapeHTML(resp)}</span></p>`;
    }
    return `<p class="prayer-text">${escapeHTML(lead)}</p>`;
  }
  ...
  function renderProseLines(text) {
    // Split after `.` or `;` + whitespace. Deliberately avoids a lookbehind
    // regex: on Safari < 16.4 a lookbehind in a regex LITERAL is a parse-time
    // SyntaxError that would kill this whole file — a blank prayer page on
    // older iPads. The control-char marker can't appear in prayer text.
  ...
  function paintingFrameHTML(picked, allPaintings, frameClass) {
    ...
    // URLs in mysteries.json are already percent-encoded — passing them
    // through encodeURI would double-encode the % signs (ges%C3%B9 →
    // ges%25C3%25B9) and 404 every such image. Only escape for the HTML
    // attribute context.
    const safeUrl = picked.url ? escapeHTML(picked.url) : "";
    ...
    switch (step.type) {
      case "our_father":
        return `
          ...
          ${leadRespHTML(step.lead, step.resp)}
          ${step.note ? `<p class="prayer-note">${escapeHTML(step.note)}</p>` : ""}
          ${guideWhisperHTML(step)}`;
      case "hail_mary":
      ...
      case "glory_fatima":
        return `
          ...
          <div class="glory-fatima">
            ${leadRespHTML(step.gloryLead, step.gloryResp)}
            ${leadRespHTML(step.fatimaLead, step.fatimaResp)}
          </div>
          ${guideWhisperHTML(step)}`;
      default:
        return "";
  ...
  function renderStep(step, container) {
    ...
    switch (step.type) {
      ...
      case "announce": {
        ...
        const q  = step.quote || {};
        // Open the Scripture with a gilded illuminated initial.
        const scText = sc.text || "";
        const scHtml = scText
          ? `<span class="versal">${escapeHTML(scText.slice(0, 1))}</span>${escapeHTML(scText.slice(1))}`
          : "";
        html = `
          <div class="stage-inner">
            <div class="announce">
              ...
              <p class="announce-scripture">
                <span class="ref">${escapeHTML(sc.reference || "")}</span>${scHtml}
              </p>
      ...
      case "hail_holy_queen":
        html = `
          <div class="stage-inner">
            ...
            <p class="prayer-text long">${renderProseLines(step.text)}</p>
            <div class="versicle hhq-versicle">
              <p class="versicle-line"><span class="speaker">℣.</span>${escapeHTML(step.versicleLeader)}</p>
              <p class="versicle-line"><span class="speaker">℟.</span>${escapeHTML(step.versicleResponse)}</p>
            </div>
            ${guideWhisperHTML(step)}
          </div>`;
        break;
          ...
          <div class="saint-bio">
            ...
            <details class="saint-bio-more">
              ...
              <div class="saint-bio-body">
                ...
                ${bio.wiki ? `<p class="saint-bio-link"><a href="${escapeHTML(bio.wiki)}" target="_blank" rel="noopener noreferrer">Read on Wikipedia ↗</a></p>` : ""}
              </div>
            </details>
          </div>` : "";
        html = `
      ...
      case "amen":
        html = `
          <div class="stage-inner">
            ...
            <span class="rule rule-tight end-rule" aria-hidden="true"></span>
            <p class="end-tagline">Ad Iesum per Mariam</p>
            <div class="end-actions">
              <a href="prayer.html${escapeHTML(location.search)}" class="end-link">Pray again</a>
              <a href="index.html" class="end-link end-link-quiet">Return home</a>
            </div>
          </div>`;
        break;
  ...
  function swapToNextAlternative(imgEl, frameId) {
    ...
    imgEl.src = next.url || "";   // already percent-encoded in the JSON
    return true;
  }
  ...
  function _onPaintingLoad(imgEl, frameId) {
    ...
    // Reject extremely portrait paintings — they look like a thin vertical
    // strip in the gold frame. Skip the fade-in for this one; the next
    // alternative will fade in on its own load. Threshold 0.50 (not 0.55):
    // the great tall altarpieces — Titian's Frari Assumption is 0.54, El
    // Greco's Assumption 0.51 — deserve to be seen; true "strips" (column
    // details, banner crops) sit well below 0.5.
  ...
  function preloadImage(url) {
    ...
    img.onload = () => {
      _loadedUrls.add(url);
      // Eagerly decode so the next display is paint-ready, not just cached
      // — kills the "pop-in" flash when navigating between decades.
      if (img.decode) img.decode().catch(() => {});
    };
    img.src = url;   // already percent-encoded in the JSON
  }
  ...
  /** localStorage.getItem that never throws — hardened browser profiles
   *  (cookies fully blocked, some private modes) throw on ANY storage
   *  access, which would otherwise kill the whole app at load time. */
  function lsGet(key) {
  ...
  // When nothing is stored, fall back to the QtWebEngine compensation
  // factor (window.__lrScale, set by the inline head script) — 1 on every
  // normal browser, so this is a no-op everywhere except qutebrowser.
  function autoScaleDefault() {
    const s = window.__lrScale;
    return (typeof s === "number" && s > 1) ? s : 1;
  }

  function readFontScale() {
    const raw = lsGet(FONT_SCALE_KEY);
    const stored = parseFloat(raw != null ? raw : String(autoScaleDefault()));
  ...
  const STEP_DURATIONS = {
    ...
    hail_holy_queen: 34, closing_prayer: 22,
  ...
  const GUIDED_KEY = "lr.guided";

  // ---------------------------------------------------------------------
  // Guided Rosary — a quiet teaching line beneath each prayer for people
  // praying the Rosary for the first time (or returning after years).
  // Tells them where their fingers are on the beads and what comes next.
  // Toggled in Settings, with the G key, or entered via ?guided=1.
  // ---------------------------------------------------------------------

  let _guidedOn = false;

  /** The whispered rubric for a step — "" when the step needs none. */
  function guideText(step) {
    switch (step.type) {
      case "sign":
        return "Hold the crucifix between your fingers and cross yourself slowly — forehead, heart, left shoulder, right. The glowing bead in the corner map will show you where you are, all the way through.";
      case "creed":
        return "Still holding the crucifix, profess the faith of the whole Church. Read it calmly — there is no rush, tonight or ever.";
      case "our_father":
        return step.decade
          ? "Your fingers rest on the large bead. One Our Father opens the decade and roots it in the Father's love."
          : "Move up to the first large bead above the crucifix. This Our Father is traditionally offered for the Pope's intentions.";
      case "hail_mary":
        if (!step.decade)
          return "Three small beads follow — one Hail Mary on each, asking for growth in faith, in hope, and in charity.";
        if (step.hailMaryNumber === 1)
          return "Now ten small beads, one Hail Mary on each. Keep the scene of the mystery before your eyes while your lips carry the words.";
        if (step.hailMaryNumber === 10)
          return "The last small bead of this decade — the Glory Be follows.";
        return "";
      case "glory_be":
        return "On the chain just before the medal, a short praise of the Trinity — the doorway into the first mystery.";
      case "announce":
        return "Announce the mystery aloud, then pause a moment. Look at the painting; let the scene rise in your imagination before you pray on.";
      case "glory_fatima":
        // The final decade has no "next large bead" — the medal awaits.
        return step.beadId === "bead-d5-hm-10"
          ? "The last decade closes: the Glory Be, then the prayer Our Lady taught at Fatima. The medal — and the Hail, Holy Queen — await."
          : "The decade closes: the Glory Be, then the prayer Our Lady taught at Fatima. Your fingers travel on to the next large bead.";
      case "hail_holy_queen":
        return "The loop is complete — you are back at the medal. The whole Rosary gathers into this hymn to Mary and its versicle.";
      case "closing_prayer":
        return "One final prayer draws everything together — that what you have contemplated, you may live.";
      case "st_michael":
        return "A traditional addition after the Rosary — the Church asks St. Michael's protection.";
      case "saint_of_day":
        return "One last intercession: the saint the Church remembers today prays with you.";
      case "closing_sign":
        return "End where you began — the Sign of the Cross, held on the crucifix. Carry one image from today's mysteries with you.";
      default:
        return "";
    }
  }

  /** Rendered whisper block, or "" when guidance is off / step is silent. */
  function guideWhisperHTML(step) {
    if (!_guidedOn) return "";
    const t = guideText(step);
    return t ? `<p class="guide-whisper">${escapeHTML(t)}</p>` : "";
  }

  /** Persisted prayer language: "en" (default) or "la" (Latin). */
  function readLang() {
    return lsGet(LANG_KEY) === "la" ? "la" : "en";
  }

  function readAutoAdvance() {
    return lsGet(AUTO_KEY) === "1";
  }
  function readAutoSpeed() {
    const v = lsGet(SPEED_KEY);
  ...
  function stepDurationMs(step, speedKey) {
    ...
    // Guidance adds a line of reading — give it a few unhurried seconds.
    const extra = (_guidedOn && step && guideText(step)) ? 4 : 0;
    const mult = SPEED_MULTIPLIERS[speedKey] ?? 1.0;
    return Math.round((base + extra) * mult * 1000);
  }
  ...
  function fitAnnouncementPainting() {
    ...
    // The stage's big bottom padding keeps long PRAYER text clear of the
    // corner chrome (rosary widget, progress bar). But the painting is
    // CENTRED — the chrome sits in the far corners, so reserving the full
    // 190–200px below it just shrinks the art and leaves dead space. On
    // comfortable screens, reclaim most of it. Genuinely short windows
    // (≤800px, already hand-tuned via media queries) are left untouched.
    const roomy = window.innerHeight > 800;
    const bottomReserve = roomy ? Math.min(padBot, 90) : padBot;
    const availableVertical = window.innerHeight - padTop - bottomReserve;
    ...
    const rawBuffer = Number.isFinite(cssBuffer) ? cssBuffer : 60;
    // Trim the safety gap on comfortable screens too — 60px of air around
    // an already-centred frame is what made the art feel small.
    const buffer = roomy ? Math.min(rawBuffer, 24) : rawBuffer;
    ...
    // Overflow guard. The maths above is good but it makes assumptions —
    // about how Scripture wraps, about font metrics, about whether the
    // Garamond webfont has arrived yet. If for any of those reasons the
    // rendered announce block actually spills past the bottom of the
    // viewport, shrink the painting by exactly that overflow. A leader
    // should NEVER have to scroll on the announcement page.
    const SAFE_GAP = 10;
    for (let pass = 0; pass < 3; pass++) {
      const overflow = announce.getBoundingClientRect().bottom
                       - (window.innerHeight - SAFE_GAP);
      if (overflow <= 0) break;
      const cur = parseFloat(img.style.maxHeight) || (target * imageScale);
      const shrunk = Math.max(180, cur - overflow - 1);
      if (shrunk >= cur) break;
      img.style.maxHeight = shrunk + "px";
    }
  }
  ...
  async function loadData() {
    // `no-cache` forces a revalidation on every load, so editing any JSON
    // file in data/ and simply reloading always shows the new content —
    // the browser can't silently serve a stale copy from disk cache.
    const opts = { cache: "no-cache" };
    const [prayersRes, mysteriesRes, saintsRes, biosRes] = await Promise.all([
      fetch("data/prayers.json", opts),
      fetch("data/mysteries.json", opts),
      ...
      fetch("data/saints.json", opts).catch(() => null),
      fetch("data/saint-bios.json", opts).catch(() => null)
    ]);
  ...
  async function startPrayerPage() {
    ...
    const fullRosary = ["full", "15", "full15"].indexOf(
      (_params.get("rosary") || "").toLowerCase()) !== -1;
    let mysteryKey = _params.get("mystery");
    if (!fullRosary &&
        !["joyful", "sorrowful", "glorious", "luminous"].includes(mysteryKey)) {
      mysteryKey = getMysteryForDate(new Date());
    }

    // Guided mode: the URL parameter wins for this visit (entry links from
    // the home and guide pages); otherwise the user's persisted preference.
    // Only an explicit toggle (Settings or the G key) persists the choice.
    const _gp = _params.get("guided");
    _guidedOn = _gp != null
      ? (_gp === "1" || _gp === "true" || _gp === "")
      : lsGet(GUIDED_KEY) === "1";
    ...
    // A "movement" is one set of five decades. The classic Rosary is one
    // movement; the traditional fifteen-decade Rosary is three — Joyful,
    // Sorrowful, Glorious. The Luminous mysteries are the modern (2002)
    // addition and are deliberately NOT part of the fifteen.
    const FIFTEEN = ["joyful", "sorrowful", "glorious"];
    let movements;
    if (fullRosary) {
      movements = FIFTEEN
        .map(k => {
          const set = data.mysteries[k];
          return set ? { mysteryKey: k, mysterySet: set, sessionPicks: pickSessionAssets(set) } : null;
        })
        .filter(Boolean);
    } else {
      const set = data.mysteries[mysteryKey];
      movements = set ? [{ mysteryKey, mysterySet: set, sessionPicks: pickSessionAssets(set) }] : [];
    }
    if (!movements.length) {
    ...
    // Prayer language. The text is rebuilt in place when this changes, so
    // the leader keeps their exact spot and the session's paintings/quotes.
    let lang = readLang();
    const prayerSetFor = (l) => (data.prayers && data.prayers[l]) || data.prayers.en;

    let steps = buildSteps(prayerSetFor(lang), movements, saintPrayer, saintBio);
    function rebuildSteps() {
      steps = buildSteps(prayerSetFor(lang), movements, saintPrayer, saintBio);
    }

    // Warm the cache for every painting this session will show, so the
    // transitions between decades are instant rather than a network wait.
    movements.forEach(mv => mv.sessionPicks.forEach(pick => {
      if (pick && pick.painting && pick.painting.url) preloadImage(pick.painting.url);
    }));
    ...
    // Map decade number → step index of its FIRST announcement. (In the
    // fifteen-decade Rosary each number 1–5 recurs once per set; jumping
    // to the first occurrence means a bead tap returns to the start of
    // the loop rather than leaping to the Glorious mysteries.)
    const decadeAnnounceIdx = {};
    steps.forEach((s, i) => {
      if (s.type === "announce" && s.decadeNumber &&
          decadeAnnounceIdx[s.decadeNumber] == null) {
    ...
    const guidedToggleEl  = document.getElementById("settings-guided-toggle");
    ...
    function refreshSettingsUI() {
      ...
      if (guidedToggleEl) {
        guidedToggleEl.setAttribute("aria-pressed", _guidedOn ? "true" : "false");
        const glabel = guidedToggleEl.querySelector(".settings-toggle-label");
        if (glabel) glabel.textContent = _guidedOn ? "On" : "Off";
      }
    ...
    // Guidance toggle — declared as a function so the G key shares it.
    function toggleGuidance() {
      _guidedOn = !_guidedOn;
      persist(GUIDED_KEY, _guidedOn ? "1" : "0");
      refreshSettingsUI();
      lastStep = null;        // force a full re-render with/without whispers
      show(cursor);
    }
    guidedToggleEl?.addEventListener("click", (e) => {
      e.stopPropagation();
      toggleGuidance();
    });
    ...
    function show(idx) {
      ...
      const applyStep = () => {
        ...
        // Auto-hide the keyboard legend once, as soon as the user advances
        // past the Sign of the Cross (cursor 0). The Apostles' Creed lives
        // on cursor 1 and is long — keeping the legend visible there causes
        // the last lines of the Creed to overlap the legend at the bottom
        // of short viewports. The user can always summon it back via the
        // chevron toggle in the bottom-right.
        if (!kbHintAutoHidden && cursor > 0) {
      ...
      //
      // EXCEPTION: when the user is contemplating a painting in the
      // lightbox, the VT animation would visibly play behind the open
      // overlay every time auto-advance ticks (or the user presses a
      // nav key). Skip the animation in that case — the stage updates
      // instantly under the lightbox, and the user sees the morph the
      // next time they navigate after closing the lightbox.
      if (!usePartial && document.startViewTransition && !isLightboxOpen()) {
        // Advancing again before the previous transition settles aborts it
        // (a leader pressing space quickly, or a fast auto-advance). That's
        // harmless — applyStep still runs — but the aborted transition
        // rejects .ready; swallow it so it isn't an unhandled rejection.
        const vt = document.startViewTransition(applyStep);
        vt && vt.ready && vt.ready.catch(() => {});
      } else {
        applyStep();
      }
    }
    ...
    // When Garamond finishes loading, text re-flows to the webfont's
    // metrics — which can make the announce block taller than the fitter
    // first measured. Re-fit so the painting is sized against the real
    // text, not the temporary fallback serif. (Resolves immediately on
    // any later announce render if fonts are already in.)
    ...
    // ---- Prayer language (English / Latin) ----
    // Latin is the Rosary's traditional tongue; both are first-class here.
    // Switching rebuilds the step list and re-renders the current step, so
    // the leader never loses their place mid-prayer.
    function refreshLangUI() {
      document.querySelectorAll("[data-lang-radio]").forEach(b => {
        b.setAttribute("aria-checked",
          b.dataset.langRadio === lang ? "true" : "false");
      });
    }

    function setLanguage(nextLang) {
      nextLang = nextLang === "la" ? "la" : "en";
      if (nextLang === lang) return;
      lang = nextLang;
      persist(LANG_KEY, lang);
      document.documentElement.setAttribute("lang", lang);
      rebuildSteps();
      lastStep = null;     // never partial-update across a language change
      show(cursor);        // same place, new language
      refreshLangUI();
    }
    function toggleLanguage() {
      setLanguage(lang === "en" ? "la" : "en");
    }

    document.querySelectorAll("[data-lang-radio]").forEach(b => {
      b.addEventListener("click", (e) => {
        e.stopPropagation();
        setLanguage(b.dataset.langRadio);
      });
    });
    ...
    // ---- Immersive idle mode ----
    // After a short stillness the chrome dissolves and the cursor vanishes
    // — nothing remains but the prayer and the painting. The faintest
    // motion brings it gently back. With auto-advance on, this turns the
    // screen into a pure, hands-free contemplation.
    (function setupImmersion() {
      ...
      function immerse() {
        if (!canImmerse()) { scheduleIdle(); return; }
        if (immersed) return;
        immersed = true;
      ...
      function scheduleIdle() {
        ...
        idleTimer = setTimeout(immerse, IDLE_MS);
      }
      function wake() {
        if (immersed) {
          immersed = false;
    ...
    window.addEventListener("keydown", (e) => {
      ...
      switch (e.key) {
        ...
        case "0":
          ...
          break;
        case "Escape":
          // An open settings panel captures Esc — close it and stay put.
    ...
    // Click navigation (good for tablets / when leading by tap):
    // the right half of the screen goes forward, the left half goes back.
    ...
    document.addEventListener("click", (e) => {
      ...
      if (e.target.closest(".top-right")) return;    // exit link
