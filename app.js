...
(function () {
  ...
  function buildSteps(prayers, mysterySet, sessionPicks, saintPrayer) {
    const steps = [];
    ...
    // Saint of the day intercession (after "Let us pray…", before St. Michael
    // and the closing Sign of the Cross).
    if (saintPrayer) {
      steps.push({
        type: "saint_of_day",
        text: saintPrayer,
  ...
  async function loadData() {
    const [prayersRes, mysteriesRes, saintsRes] = await Promise.all([
      fetch("data/prayers.json"),
      fetch("data/mysteries.json"),
      // saints.json is optional — if it fails, we fall back to a generic
      // Marian intercession via todaysSaintPrayer().
    ...
    return { prayers, mysteries, saints };
  }
  ...
  async function startPrayerPage() {
    ...
    const saintPrayer = todaysSaintPrayer(data.saints, new Date());
    const steps = buildSteps(data.prayers, mysterySet, sessionPicks, saintPrayer);
