...
(function () {
  ...
  function readImageScale() {
    const raw = localStorage.getItem(IMG_SCALE_KEY);
    const stored = parseFloat(raw != null ? raw : String(autoScaleDefault()));
    if (Number.isNaN(stored)) return 1;
  ...
  function readLang() {
    return localStorage.getItem(LANG_KEY) === "la" ? "la" : "en";
  }
  ...
  // ---------------------------------------------------------------------
  // 7g². Ambient sacred music — Gregorian chant under the prayer
  //
  // Off by default. When the user enables it in settings, a single looping
  // <audio> element plays the chosen track at the chosen volume. Switching
  // tracks or toggling on/off cross-fades over a beat so the change never
  // jolts the prayer. Volume and choice persist; the chant is never
  // imposed on the next session.
  // ---------------------------------------------------------------------

  const AUDIO_TRACKS = [
    { id: "salve",   name: "Salve Regina",         note: "Marian antiphon",
      url: "https://commons.wikimedia.org/wiki/Special:FilePath/Salve_Regina.ogg" },
    { id: "avemaria",name: "Ave Maria",            note: "Schola Gregoriana",
      url: "https://commons.wikimedia.org/wiki/Special:FilePath/Schola_Gregoriana-Ave_Maria.ogg" },
    { id: "pater",   name: "Pater Noster",         note: "Schola Gregoriana",
      url: "https://commons.wikimedia.org/wiki/Special:FilePath/Schola_Gregoriana-Pater_Noster.ogg" },
    { id: "tantum",  name: "Tantum Ergo",          note: "Gregorian, Mode I",
      url: "https://commons.wikimedia.org/wiki/Special:FilePath/Tantum_Ergo_I_Gregorian.ogg" },
    { id: "veni",    name: "Veni Creator Spiritus",note: "Pentecost hymn",
      url: "https://commons.wikimedia.org/wiki/Special:FilePath/Veni.creator.spiritus.ogg" },
    { id: "kyrie",   name: "Kyrie",                note: "Mass XI · Gregorian",
      url: "https://commons.wikimedia.org/wiki/Special:FilePath/Kyrie_XI.ogg" },
    { id: "gloria",  name: "Gloria",               note: "Gregorian chant",
      url: "https://commons.wikimedia.org/wiki/Special:FilePath/GloriaGregorianChant.ogg" }
  ];

  const AUDIO_ON_KEY     = "lr.audioOn";
  const AUDIO_TRACK_KEY  = "lr.audioTrack";
  const AUDIO_VOLUME_KEY = "lr.audioVolume";

  function readAudioOn() { return localStorage.getItem(AUDIO_ON_KEY) === "1"; }
  function readAudioTrackId() {
    const id = localStorage.getItem(AUDIO_TRACK_KEY);
    return AUDIO_TRACKS.some(t => t.id === id) ? id : "salve";
  }
  function readAudioVolume() {
    const raw = parseFloat(localStorage.getItem(AUDIO_VOLUME_KEY));
    if (!isFinite(raw)) return 0.4;
  ...
  function createAmbientAudio() {
    ...
    let on        = readAudioOn();
    let trackId   = readAudioTrackId();
    let volume    = readAudioVolume();
    let fadeRaf   = 0;
    let lastError = null;
    ...
    function fadeTo(targetVol, durationMs, after) {
      ...
      function step(now) {
        ...
        el.volume = Math.max(0, Math.min(1, start + (targetVol - start) * eased));
        if (k < 1) {
    ...
    function setSource(track) {
      ...
      if (el.dataset.trackId === track.id) return;
    ...
    function start() {
      ...
      if (p && typeof p.then === "function") {
        p.catch(() => { /* autoplay blocked — caller can retry */ });
      }
      fadeTo(volume, 1200);
    }
    function stop() {
      fadeTo(0, 900, () => { el.pause(); });
    }

    el.addEventListener("error", () => {
      lastError = "could not load";
      api.notify && api.notify("error");
    });
    ...
    const api = {
      ...
      setOn(next) {
        on = !!next;
        persist(AUDIO_ON_KEY, on ? "1" : "0");
        if (on) start(); else stop();
        api.notify && api.notify("on");
      },
      setTrack(id) {
        if (!AUDIO_TRACKS.some(t => t.id === id)) return;
        trackId = id;
        persist(AUDIO_TRACK_KEY, id);
        if (on) {
          fadeTo(0, 350, () => {
            setSource(trackById(trackId));
            el.currentTime = 0;
            const p = el.play();
        ...
        api.notify && api.notify("track");
      },
      setVolume(v) {
        volume = Math.min(1, Math.max(0, +v || 0));
        persist(AUDIO_VOLUME_KEY, String(volume));
        if (on) el.volume = volume;
        api.notify && api.notify("volume");
      },
      // The first user click anywhere on the prayer page satisfies most
      // browsers' autoplay policy; call this then so playback resumes
      // without the user having to toggle the switch a second time.
      kick() {
        if (on && el.paused) {
          const p = el.play();
          if (p && typeof p.then === "function") p.catch(() => {});
  ...
  async function startPrayerPage() {
    ...
    const audioToggleEl  = document.getElementById("settings-audio-toggle");
    const audioListEl    = document.getElementById("audio-tracklist");
    const audioVolumeEl  = document.getElementById("settings-audio-volume");

    if (ambient && audioListEl) {
      audioListEl.innerHTML = ambient.tracks.map(t => `
        <li>
          <button type="button" class="audio-track" role="radio"
                  aria-checked="false" data-track-id="${t.id}">
            <span class="audio-track-main">
              <span class="audio-track-name">${escapeHTML(t.name)}</span>
              <span class="audio-track-note">${escapeHTML(t.note || "")}</span>
            </span>
            <span class="audio-track-indicator" aria-hidden="true"></span>
          </button>
        </li>`).join("");
      ...
      audioListEl.addEventListener("click", (e) => {
        ...
        if (!btn) return;
        if (!ambient.isOn()) ambient.setOn(true);
        ambient.setTrack(btn.dataset.trackId);
      });
    }
    if (ambient && audioVolumeEl) {
      const updateVolumeFill = () => {
        audioVolumeEl.style.setProperty("--vp", audioVolumeEl.value + "%");
      };
      audioVolumeEl.value = Math.round(ambient.volume() * 100);
      updateVolumeFill();
      audioVolumeEl.addEventListener("input", (e) => {
        e.stopPropagation();
        ambient.setVolume(audioVolumeEl.value / 100);
        updateVolumeFill();
      });
    }
    if (ambient && audioToggleEl) {
      audioToggleEl.addEventListener("click", (e) => {
        e.stopPropagation();
        ambient.setOn(!ambient.isOn());
      });
    }
    if (ambient) {
      ambient.notify = () => refreshSettingsUI();
      // Browsers block autoplay until the user interacts. The user's very
      // next click anywhere on the page lets a previously-on track resume.
      window.addEventListener("pointerdown", () => ambient.kick(), { once: true });
      window.addEventListener("keydown",     () => ambient.kick(), { once: true });
    }
    ...
    function refreshSettingsUI() {
      ...
      if (ambient && audioToggleEl) {
        const on = ambient.isOn();
        audioToggleEl.setAttribute("aria-pressed", on ? "true" : "false");
        const label = audioToggleEl.querySelector(".settings-toggle-label");
        if (label) label.textContent = on ? "On" : "Off";
      }
      if (ambient && audioListEl) {
        const id = ambient.trackId();
        const err = ambient.hasError();
        audioListEl.querySelectorAll("[data-track-id]").forEach(b => {
          const selected = b.dataset.trackId === id;
          b.setAttribute("aria-checked", selected ? "true" : "false");
          b.classList.toggle("is-active", selected);
          b.classList.toggle("is-playing", selected && ambient.isOn() && !err);
          b.classList.toggle("has-error",  selected && err);
        });
      }
      if (ambient && audioVolumeEl) {
        const v = Math.round(ambient.volume() * 100);
        if (parseInt(audioVolumeEl.value, 10) !== v) audioVolumeEl.value = v;
      }
    }
